...
public class CartListService {
    ...
    @Transactional
    public void orderCartItems(List<CartOrderDto> cartOrderDtoList, Principal principal) {//장바구니 내용 주문
        Member member = getMember(principal);
    ...
    @Transactional
    public void changeItemCount(Long itemId, int count) {//주문수량 수정
    ...
    @Transactional
    public void cancelOrderItem(Long itemId) {//주문 취소
        orderService.deleteOrderItemById(itemId);
    }

    private Member getMember(Principal principal) {//주문자 정보 확인
    ...
    private void changeItemStock(int count, OrderItem orderItem) {//재고 변경
