...
public class FileService {
    ...
    private String fileUploadPath;//파일 저장 경로
    ...
    public UploadFile storeFile(MultipartFile multipartFile) throws IOException {
        ...
        multipartFile.transferTo(new File(getFullFileUploadPath(storeFileName)));//경로에 파일 생성

        return new UploadFile(originalFilename, storeFileName, fileUploadUrl);
    }

    private String createStoreFileName(String originalFilename) { //파일 UUID로 rename
        String uuid = UUID.randomUUID().toString();
        String ext = extractExt(originalFilename);
        return uuid + "." + ext;
    }

    private String extractExt(String originalFilename) { //확장자 저장
